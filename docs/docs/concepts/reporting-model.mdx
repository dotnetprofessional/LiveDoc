...
graph LR
    subgraph Producers
        V["@swedevtools/livedoc-vitest<br/>(TypeScript)"]
        X["SweDevTools.LiveDoc.xUnit<br/>(C#)"]
    end
    subgraph Consumers
        Viewer["LiveDoc Viewer<br/>(React app)"]
        VSCode["VS Code Extension"]
    end
    V -->|"Protocol 1.0"| Viewer
    V -->|"Protocol 1.0"| VSCode
    X -->|"Protocol 1.0"| Viewer
    X -->|"Protocol 1.0"| VSCode
```
...
| Field              | Description                                           |
| ------------------ | ----------------------------------------------------- |
| `protocolVersion`  | Always `"1.0"` for the current protocol               |
...
| `kind`        | Discriminator: `"Feature"`, `"Specification"`, or `"Container"`    |
...
| `background`  | Optional background scenario (Feature kind only)                   |
...
The current reporting model uses **protocol version 1.0**. This version was designed with several principles:
...
Both the TypeScript/Vitest SDK and the C#/xUnit SDK produce protocol 1.0 output. This is what enables the shared Viewer — it doesn't matter which language wrote the tests, the structured output is identical.
...
The mapping is handled internally by each SDK's reporter. Test authors don't need to think about the model — they write tests using their SDK's natural API, and the reporter transforms the results into protocol 1.0 format.
...
- **Protocol 1.0** is the current standard — both SDKs produce it, enabling the shared Viewer.
- Titles are templates; the UI applies **bindings** to render concrete values with highlighting.

## Next Steps

- **Hands-on**: [Tutorial: Beautiful Tea](./tutorial-beautiful-tea.mdx) — see all these concepts come together in a complete example
- **For contributors**: [REPORTER_MODEL_V1.md](https://github.com/dotnetprofessional/LiveDoc/blob/main/REPORTER_MODEL_V1.md) — full model specification and TypeScript interfaces
