...
```mermaid
graph TD
    Spec["📄 Spec<br/>(Word / Confluence)"] -->|"separate artifact"| Code["💻 Code<br/>(source files)"]
    Code -->|"separate artifact"| Tests["🧪 Tests<br/>(assertions only)"]
    Spec -.-|"❌ Stale within weeks"| Drift["Documentation Drift"]
    Code -.-|"✅ Keeps evolving"| Reality["Reality"]
    Tests -.-|"⚠️ Pass/fail only"| NoDoc["No documentation value"]
```
...
```mermaid
graph LR
    Spec["📝 Executable Specification<br/>(.Spec.ts / .cs)"] -->|"runs as"| Test["🧪 Test"]
    Spec -->|"outputs"| Docs["📖 Documentation"]
    Test -->|"catches"| Regressions["✅ Regressions"]
    Docs -->|"always"| Accurate["✅ Accurate"]
```
