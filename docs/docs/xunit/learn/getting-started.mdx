...
<Tabs>
  <TabItem value="ai-agent" label="AI Agent" default>

Give GitHub Copilot, OpenAI Codex, Claude Code, Cursor, Roo Code, Windsurf, or
another repository-aware agent this prompt:
...
The agent will confirm the project name, Viewer publishing, Microsoft Code
Coverage, thresholds, tag-based partial testing, and whether to add a
representative LiveDoc test. It also asks which AI tools you use and installs
version-matched skills for each one inside the repository. It creates
PowerShell and Bash launchers for normal and coverage runs. After it finishes,
continue to [Step 4](#step-4-run-it-) to inspect the result.
...
The first command runs tests normally. The second requests Microsoft Code
Coverage in Cobertura format, including branch data, without requiring the full
collector syntax.
