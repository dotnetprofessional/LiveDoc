...
If you use GitHub Copilot, Claude Code, Cursor, Roo Code, or Windsurf, give the
assistant this prompt. LiveDoc does not need to be installed yet:

> Read https://livedoc.swedevtools.com/ai/setup.md and configure this xUnit
> project for LiveDoc with Viewer publishing and full Microsoft Code Coverage
> including branch data. Preserve existing runsettings.

The bootstrap page teaches the assistant which packages to install, the correct base classes and attributes,
self-documenting value extraction, Viewer configuration, coverage collector
choice, dependencies, diagnostics, and validation commands. Review the changes,
then continue to [Step 4](#step-4-run-it-) to run the suite.

**[See the AI project setup workflow →](../../guides/ai-project-setup.mdx)**

After setup, optionally install the reusable xUnit skill with
`dotnet msbuild -t:LiveDocInstallSkills` for future AI authoring sessions.

### Manual Setup

Continue with Step 3 to create the test yourself.
...
:::tip Console Logger vs. Test Explorer
The console logger shows the **Feature → Scenario/Rule** hierarchy at a glance.
For **step-level detail** (Given / When / Then), click a test in Visual Studio's
Test Explorer — the full BDD output appears in the detail panel.
:::
...
- **End-to-end testing**: [Journey Testing](../guides/journey-testing.mdx) — test full API flows with annotated `.http` files
