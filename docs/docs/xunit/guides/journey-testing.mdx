...
create a small subclass that specifies your project paths.
...
public class JourneyServerFixture : JourneyFixtureBase
{
    public JourneyServerFixture()
        : base(
            ...
            journeysDir: "../../journeys"              // relative to test .csproj
        ) { }
}
```

**That's it.** The `JourneyFixtureBase` provides:

| Concern | Default |
| --- | --- |
| **Port** | Random ephemeral port via `TcpListener` |
| **Server start** | `dotnet run --project {path} --no-launch-profile` |
| **Environment** | `ASPNETCORE_URLS=http://localhost:{port}`, `ASPNETCORE_ENVIRONMENT=Test` |
| **Startup detection** | Monitors stdout for Kestrel's "Now listening on" signal |
| **httpYac variables** | `baseUrl` automatically set to `http://localhost:{port}` |
| **Capture mode** | Checks `JOURNEY_CAPTURE=true` env var, saves `.Response.json` files |
| **Cleanup** | Kills server process tree on dispose |

### Adding Custom Variables {#custom-vars}
...
{
    public JourneyServerFixture()
        : base("../../src/MyProject.Api", "../../journeys") { }
...
### Custom Server Configuration {#custom-config}

Override `ConfigureServerProcess()` to set additional environment variables or
...
public class JourneyServerFixture : JourneyFixtureBase
{
    ...
    {
        base.ConfigureServerProcess(psi); // sets ASPNETCORE_URLS + ASPNETCORE_ENVIRONMENT
...
:::tip
The `TempDataDir` property provides a unique temporary directory created for each
test run — perfect for isolating server data so test runs don't interfere with
each other.
:::

### Custom Startup Detection {#custom-startup}

If your server doesn't use standard Kestrel output (or you need to detect a
different startup signal), override `IsServerReady()`:
...
Override `OnServerOutputLine()` to capture values from server startup logs
(e.g., generated tokens, connection strings):
...
public class JourneyServerFixture : JourneyFixtureBase
{
    ...
    protected override void OnServerOutputLine(string line)
    {
        ...
        {
            var marker = "Admin token:";
            var idx = line.IndexOf(marker, StringComparison.OrdinalIgnoreCase);
            if (idx >= 0)
                _adminToken = line[(idx + marker.Length)..].Trim();
        }
    }

    protected override Dictionary<string, string> GetHttpYacVariables()
    {
        var vars = base.GetHttpYacVariables();
        vars["adminToken"] = AdminToken;
        return vars;
    }
}
```
...
All of these ship in the `SweDevTools.LiveDoc.xUnit.Journeys` namespace:
...
| `JsonAssertionException` | Thrown on JSON comparison failure |
| `JourneyAssertionException` | Thrown on step assertion failure |

### Full Override Reference {#override-reference}

| Virtual Member | Type | Default | When to Override |
| --- | --- | --- | --- |
| `ConfigureServerProcess(psi)` | `void` | Sets `ASPNETCORE_URLS` + `ASPNETCORE_ENVIRONMENT=Test` | Custom env vars, non-standard port config |
| `IsServerReady(line)` | `bool` | Checks for "Now listening on" / "Application started" | Non-Kestrel server |
| `GetHttpYacVariables()` | `Dictionary` | `{ "baseUrl": BaseUrl }` | Additional httpYac variables |
| `StartupTimeout` | `TimeSpan` | 30 seconds | Slow-starting servers |
| `HttpYacEnvironment` | `string` | `"local"` | Different httpYac env name |
| `OnServerOutputLine(line)` | `void` | No-op | Extract tokens/values from startup logs |
...
- [Configuration](../reference/configuration.mdx) — project setup and `.runsettings`
