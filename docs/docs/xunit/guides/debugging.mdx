...
Use environment variables to label the test run and enable reporting in CI:

```powershell
# PowerShell
$env:LIVEDOC_ENVIRONMENT = "ci"
...
Run with the same environment label and verbosity as CI:

```powershell
$env:LIVEDOC_ENVIRONMENT = "ci"
dotnet test --logger LiveDoc --verbosity normal
```
