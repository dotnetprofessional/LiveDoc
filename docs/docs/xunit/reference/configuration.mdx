...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <PackageReference Include="SweDevTools.LiveDoc.xUnit" Version="0.3.0" />
  </ItemGroup>
</Project>
```

LiveDoc supports both `net8.0` and `net10.0` test projects.
...
| `LIVEDOC_RUN_TYPE` | `"full"` | Use `"partial"` for a focused run layered over the latest full baseline. |
| `LIVEDOC_COVERAGE` | *(disabled)* | Explicitly request LiveDoc coverage processing. |
| `LIVEDOC_COVERAGE_PATH` | *(auto-detect)* | Custom Cobertura artifact path. |
| `LIVEDOC_DOTNET_COVERAGE_TOOL` | `dotnet-coverage` from `PATH` | Converter path for binary Visual Studio `.coverage` files. |
| `LIVEDOC_COVERAGE_THRESHOLD_LINES` | *(none)* | Non-fatal minimum line percentage. |
| `LIVEDOC_COVERAGE_THRESHOLD_BRANCHES` | *(none)* | Non-fatal minimum branch percentage. |

For incremental validation, mark stable capabilities with `[Tag]`. LiveDoc
exposes tags as xUnit `Category` traits, allowing a focused run to patch the
latest full Viewer baseline:
...
Partial runs require a running server and an existing full run for the same
project and environment. Use a full run for release validation and static
exports.
...
The package includes the LiveDoc VSTest collector. The recommended solution-wide
command requests Microsoft Code Coverage as Cobertura and includes branch data:

```powershell
dotnet test .\MySolution.sln --collect:"Code Coverage;Format=Cobertura"
```

Visual Studio binary `.coverage` output additionally requires:

```powershell
dotnet tool install --global dotnet-coverage
```

See [Code Coverage](../../viewer/guides/code-coverage.mdx) for the dependency
matrix, XPlat/Coverlet alternative, custom runsettings, and diagnostics.
