...
Export a `TestRunV3` JSON file directly to disk after the test run — no server needed. This is the recommended approach for CI pipelines and for generating [static HTML reports](../../viewer/guides/static-export.mdx).

#### Environment Variable {#export-env-var}

Set the `LIVEDOC_EXPORT_PATH` environment variable to enable JSON export:
...
#### Environment Variables Reference {#export-env-vars}

| Variable | Default | Description |
|----------|---------|-------------|
| `LIVEDOC_EXPORT_PATH` | *(none — export disabled)* | File path for TestRunV3 JSON output. When set, enables export. |
| `LIVEDOC_PROJECT` | Assembly name | Project name embedded in the JSON. |
| `LIVEDOC_ENVIRONMENT` | `"local"` | Environment label (e.g., `"local"`, `"ci"`, `"staging"`). |
