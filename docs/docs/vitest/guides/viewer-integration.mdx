...
real time. `LiveDocSpecReporter` automatically discovers a running viewer and
publishes results — no extra configuration needed. For explicit control, you
can use `LiveDocViewerReporter` as a post-reporter. The viewer renders your
Gherkin features as an interactive, navigable tree.
...
The simplest approach is just `LiveDocSpecReporter` — it automatically discovers
a running viewer on your machine. No extra reporter needed:
...
If no viewer is running, the reporter silently continues with console-only
output — your tests still run normally.
...
Install the coverage provider that matches your Vitest version:

```bash
npm install --save-dev @vitest/coverage-v8
```

Enable both Vitest coverage and LiveDoc coverage evidence:
...
LiveDoc reads the in-memory coverage map and displays line, branch, function,
and statement metrics. See the shared [Code Coverage guide](../../viewer/guides/code-coverage.mdx)
for artifact fallbacks and troubleshooting.
...
| `project` | `'livedoc'` | Project name for grouping results |
...
| ------- | ----- | -------- |
| Viewer shows no results | Reporter not configured or server not discovered | Add `LiveDocSpecReporter` to config — it auto-discovers a running server |
...
| Want explicit control | Auto-discovery not desired | Use `LiveDocViewerReporter` as a post-reporter with explicit server URL |
| Coverage is absent | Provider missing or coverage not enabled | Install `@vitest/coverage-v8` and run with `--coverage` |
...
- [Code Coverage](../../viewer/guides/code-coverage.mdx) — coverage providers, thresholds, and Viewer behavior
