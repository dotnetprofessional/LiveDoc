...
a TestRunV3 JSON file, generates a static HTML report, and uploads artifacts.
</p>
...
configuration needed. Add the `export` option to write a TestRunV3 JSON file,
then use `livedoc-viewer export` to generate a shareable static HTML report.

```mermaid
graph LR
    A["Run tests<br/>npx vitest run"] --> B["TestRunV3 JSON<br/>livedoc-report.json"]
    B --> C["Static HTML<br/>livedoc-viewer export"]
    C --> D["Upload artifact<br/>report.html"]
```

## Step 1: Configure TestRunV3 Export

Add the `export` option to `LiveDocSpecReporter` to write a TestRunV3 JSON file
after each test run:
...
The TestRunV3 JSON file contains all features, scenarios, steps, pass/fail
status, timing data, error messages, tags, and attachments.

:::tip Alternative: JsonReporter (SDK model)
The `JsonReporter` post-reporter also writes JSON, but in the **LiveDoc SDK model format** — not TestRunV3. Use `export` for the standard pipeline. Use `JsonReporter` only if you need the raw SDK model for custom tooling:
...
Use `livedoc-viewer export` to convert the TestRunV3 JSON into a self-contained
