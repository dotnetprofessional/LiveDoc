...
> project for LiveDoc, the Viewer, and V8 coverage. Preserve the existing Vitest
> configuration.

The bootstrap page teaches the assistant the required dependencies, imports, reporter configuration,
file naming, coverage provider dependency, self-documenting value extraction,
and validation commands. Review the generated changes, then continue to
[Step 5](#step-5-run-it-) to run the suite.

**[See the AI project setup workflow →](../../guides/ai-project-setup.mdx)**

After setup, optionally install the reusable Vitest skill with
`npx livedoc-vitest-setup` for future AI authoring sessions.

### Manual Setup

Continue with Step 3 to configure each file yourself.
