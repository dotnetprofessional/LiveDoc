...
| `export`           | `object`           | `undefined`                | Write a TestRunV3 JSON file to disk (see [Export Options](#export-options)). |
...
Write a `TestRunV3` JSON file directly to disk after each test run — no server needed. This is the recommended approach for CI pipelines and for feeding the [static HTML export](../../viewer/guides/static-export.mdx).
...
The exported JSON file can be used with [`livedoc-viewer export`](../../viewer/reference/cli-options.mdx#export-subcommand) to generate a self-contained HTML report, or consumed by any tool that reads the TestRunV3 schema.
...
### CLI with TestRunV3 export for CI {#ci-export}

Export a `TestRunV3` JSON file for CI artifact upload or [static HTML export](../../viewer/guides/static-export.mdx):
...
The `JsonReporter` writes the raw LiveDoc SDK model to a JSON file. This is useful for custom tooling but is **not** the same format as `TestRunV3`.
