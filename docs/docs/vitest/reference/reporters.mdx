...
| `coverage`         | `object`           | `undefined`                | Attach coverage evidence and optional thresholds. Requires a Vitest coverage provider. |
...
4. **Default local viewer** — probes `http://localhost:3100/api/health`

If no server is found, the reporter silently continues with console-only output — your tests always run normally.
Use `LIVEDOC_PROJECT` and `LIVEDOC_ENVIRONMENT` to control where the run appears in the viewer.
...
| `publish.project`     | `string`  | `"livedoc"`    | Project name for grouping results  |
| `publish.environment` | `string`  | `"local"`      | Environment label                  |
| `publish.runType`     | `"full" \| "partial"` | `"full"` | Publish a complete baseline or focused partial update |

#### Coverage Options {#coverage-options}

Install `@vitest/coverage-v8` at the same version as Vitest, enable Vitest
coverage, and set `coverage.enabled` on the reporter:
...
| Option | Type | Default | Description |
| ------ | ---- | ------- | ----------- |
| `coverage.enabled` | `boolean` | `false` | Expect and publish coverage evidence |
| `coverage.path` / `coverage.artifactPath` | `string` | Auto-detect | Custom JSON summary or LCOV artifact |
| `coverage.thresholds` | `Partial<Record<"lines" \| "branches" \| "functions" \| "statements", number>>` | None | Non-fatal Viewer warnings |

See [Code Coverage](../../viewer/guides/code-coverage.mdx) for the complete
runner configuration and dependency matrix.
...
| `export.project`     | `string` | Publish config or `"livedoc"` | Project name embedded in the JSON. Falls back to `publish.project`. |
...
| `project`     | `string`  | `"livedoc"`                  | Project name for grouping results                |
