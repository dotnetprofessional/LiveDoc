...
Generate a self-contained static HTML report from a TestRunV3 JSON file. The
...
**(Required)** Path to a TestRunV3 JSON file. This is the file produced by the
...
The file must contain valid JSON with `protocolVersion: "3.0"`. If the file is
...
| ------------------ | ----- | -------- | ------------------------- | ---------------------------------------- |
| `--input <path>`   | `-i`  | Yes      | —                         | Path to TestRunV3 JSON file              |
