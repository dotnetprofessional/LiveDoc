...
```mermaid
graph LR
    A["1. Run tests"] --> B["2. TestRunV3 JSON"]
    B --> C["3. Static HTML"]
...
|------|-------------|--------|
| **Run tests** | Your test framework executes specs and exports results | `livedoc-report.json` (TestRunV3 format) |
...
| `Invalid TestRunV3 format` | Wrong JSON format | Ensure you're using the `export` option (TestRunV3), not `JsonReporter` (SDK model) |
