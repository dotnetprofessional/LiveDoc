...
if (-not (Test-Path $viewerDir)) {
    throw "Viewer folder not found at: $viewerDir"
}
...
if ($health) {
    Write-Host "LiveDoc Viewer already running at http://$HostName`:$Port (status=$($health.status))." -ForegroundColor Green
...
$health = Test-LiveDocHealth -hostName $HostName -p $Port -timeoutSec $HealthTimeoutSec
if ($health) {
    Write-Host "LiveDoc Viewer is now responding at http://$HostName`:$Port (status=$($health.status))." -ForegroundColor Green
    return
}

$cmd = "Set-Location `"$viewerDir`"; pnpm run dev"

Write-Host "Starting LiveDoc Viewer dev server on port $Port..." -ForegroundColor Cyan
