...
This skill targets **SweDevTools.LiveDoc.xUnit v0.1.8-beta5**. Before writing tests, verify the installed version matches:

```bash
dotnet list package | grep -i livedoc
```

If the installed version differs from `0.1.8-beta5`, tell the developer: *"Your LiveDoc skill files target v0.1.8-beta5 but you have vX.Y.Z installed. Run `dotnet msbuild -t:LiveDocInstallSkills` to update the skill files, or check the changelog for breaking changes."*
