...
$viewerPkgDir = Join-Path $repoRoot 'packages\viewer'

$items.Add((New-MenuItem -Label 'Publish' -HotKey 'n' -Children @(
    (New-MenuItem -Label '@swedevtools/livedoc-vitest' -HotKey '1' -Children @(
        (New-MenuItem -Label 'Local release (pack .tgz)' -HotKey '1' `
            -Action {
                Write-Host '==> Syncing skill versions...' -ForegroundColor Cyan
                & "$publishScriptsDir\sync-skill-versions.ps1"
                Write-Host '==> Building @swedevtools/livedoc-vitest...' -ForegroundColor Cyan
                Invoke-InDirectory -Path $vitestPkgDir -Action { pnpm run build }
                if ($LASTEXITCODE -ne 0) { Write-Host 'Build failed!' -ForegroundColor Red; return }
                Write-Host '==> Packing...' -ForegroundColor Cyan
                Invoke-InDirectory -Path $vitestPkgDir -Action { pnpm run pack:local }
                Sync-Releases
            }.GetNewClosure() `
    ...
    (New-MenuItem -Label '@swedevtools/livedoc-viewer' -HotKey '2' -Children @(
        (New-MenuItem -Label 'Local release (pack .tgz)' -HotKey '1' `
    ...
    (New-MenuItem -Label 'SweDevTools.LiveDoc.xUnit' -HotKey '3' -Children @(
        (New-MenuItem -Label 'Local release (pack .nupkg)' -HotKey '1' `
            -Action {
                Write-Host '==> Syncing skill versions...' -ForegroundColor Cyan
                & "$publishScriptsDir\sync-skill-versions.ps1"
