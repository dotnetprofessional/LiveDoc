...
function getOutlineTitle(title: string, exampleValues: Record<string, string>): string {
  // The title might be "Scenario Title: Example 1" or just have actual values
  // First, remove "Example N" suffix
  let outlineTitle = title.replace(/:\s*Example\s*\d+$/i, '').trim();
  
  // Also handle case where title is just "Example 1"
  if (outlineTitle.match(/^Example\s*\d+$/i)) {
    // No meaningful title, use the outline ID or first example's parent
    return outlineTitle;
  }
  
  // Try to find and replace values with placeholders
  for (const [key, value] of Object.entries(exampleValues)) {
    if (value && outlineTitle.includes(value)) {
      outlineTitle = outlineTitle.replace(new RegExp(escapeRegExp(value), 'g'), `<${key}>`);
    }
  }
...
function getTemplateSteps(scenario: Scenario): { type: string; title: string }[] {
  ...
  return scenario.steps.map(step => {
    ...
    // If this is a ScenarioOutline definition, step titles may already have <placeholders>
