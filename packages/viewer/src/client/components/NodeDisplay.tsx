import { Node, Feature, SpecKind } from '@livedoc/schema';
import { BookOpen, FileText, LayoutList, ScrollText, Target, Clock, Calendar, Globe, Tag, Layers, Info, CheckCircle2, XCircle, AlertCircle, HelpCircle, ChevronRight } from 'lucide-react';
...
// Maps kinds to icons and colors
const KIND_CONFIG: Record<string, { icon: any, label: string }> = {
  [SpecKind.Feature]: { icon: BookOpen, label: 'Feature' },
  [SpecKind.Specification]: { icon: ScrollText, label: 'Specification' },
  [SpecKind.Suite]: { icon: LayoutList, label: 'Suite' },
  [SpecKind.Scenario]: { icon: Target, label: 'Scenario' },
  [SpecKind.ScenarioOutline]: { icon: Target, label: 'Scenario Outline' },
  [SpecKind.Rule]: { icon: FileText, label: 'Rule' },
  [SpecKind.Test]: { icon: FileText, label: 'Test' },
  'default': { icon: FileText, label: 'Node' }
};
...
export function NodeDisplay({ node, variant, size = 'lg', showStats = true, onClick, contextLabel }: NodeDisplayProps) {
  ...
  // Match loosely or strictly
  const config = Object.entries(KIND_CONFIG).find(([k]) => k.toLowerCase() === kind)?.[1] || KIND_CONFIG.default;
  ...
  // Title highlighting
  const highlightValues = node.binding ? (node.binding as any).variables?.reduce((acc: any, v: any) => ({ ...acc, [v.name]: v.value?.value }), {}) : undefined;
